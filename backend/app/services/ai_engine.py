...
def _rule_based_reply(message: str, rag_block: str) -> str:
    ...
    if any(marker in lower for marker in _CYCLE_TRAINING_MARKERS):
        return (
            "Не стоит назначать лёгкую, среднюю или тяжёлую тренировку только по "
            "предполагаемой фазе или считать цикл фиксированными 28 днями. Ориентируйтесь "
            "на фактическое самочувствие сегодня: в профиле можно включить учёт цикла, а "
            "в дневном чек-ине выбрать обычную, осторожную, лёгкую нагрузку или отдых. "
