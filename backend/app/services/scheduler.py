...
async def get_schedule_overview(
    ...
    if isinstance(current, dict) and current.get("status") == "scheduled":
        ...
        if completed is not None:
            ...
            try:
                current["day_index"] = int(plan.get("day_index") or current.get("day_index") or 0) or None
            except (TypeError, ValueError):
                current["day_index"] = current.get("day_index")
            current["can_reschedule"] = False
            current["reschedule_until"] = None

            # The pure schedule includes today as both current and next. Once
            # today is completed, expose the first future occurrence instead.
