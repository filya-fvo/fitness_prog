    ...
    <section>
      ...
      <div className="my-5 space-y-3 rounded-2xl bg-tg-secondary p-4">
        ...
        <div className="grid gap-2 sm:grid-cols-3">
          ...
          <select aria-label="Оборудование" value={filters.equipment || ""} onChange={(event) => applyFilters({ ...filters, equipment: event.target.value || undefined })} className="min-h-11 rounded-xl border border-black/10 bg-tg-bg px-3 text-base"><option value="">Всё оборудование</option>{options.equipment.map((item) => <option key={item} value={item}>{item}</option>)}</select>
          <select aria-label="Качество медиа" value={filters.mediaQuality || ""} onChange={(event) => applyFilters({ ...filters, mediaQuality: (event.target.value || undefined) as MediaQuality | undefined })} className="min-h-11 rounded-xl border border-black/10 bg-tg-bg px-3 text-base"><option value="">Любое медиа</option>{Object.entries(qualityLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select>
          <select aria-label="Сложность упражнения" value={filters.difficulty || ""} onChange={(event) => applyFilters({ ...filters, difficulty: event.target.value ? Number(event.target.value) : undefined })} className="min-h-11 rounded-xl border border-black/10 bg-tg-bg px-3 text-base"><option value="">Любая сложность</option>{[1, 2, 3, 4, 5].map((item) => <option key={item} value={item}>{item} из 5</option>)}</select>
          <select aria-label="Правило учёта веса" value={filters.weightRule || ""} onChange={(event) => applyFilters({ ...filters, weightRule: (event.target.value || undefined) as WeightRule | undefined })} className="min-h-11 rounded-xl border border-black/10 bg-tg-bg px-3 text-base"><option value="">Любой учёт веса</option>{Object.entries(weightLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select>
          <select aria-label="Тег упражнения" value={filters.tag || ""} onChange={(event) => applyFilters({ ...filters, tag: event.target.value || undefined })} className="min-h-11 rounded-xl border border-black/10 bg-tg-bg px-3 text-base"><option value="">Любой тег</option>{options.tags.map((item) => <option key={item} value={item}>{item}</option>)}</select>
        </div>
      </div>
