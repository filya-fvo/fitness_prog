...
$settings = [ordered]@{
  ...
  LLM_BASE_URL = Get-Value $source "LLM_BASE_URL" "https://api.groq.com/openai/v1"
  LLM_MODEL = Get-Value $source "LLM_MODEL" "qwen/qwen3.6-27b"
  LLM_FALLBACK_MODELS = Get-Value $source "LLM_FALLBACK_MODELS"
  NUTRITION_VISION_MODEL = Get-Value $source "NUTRITION_VISION_MODEL" "qwen/qwen3.6-27b"
  AI_DAILY_LIMIT = Get-Value $source "AI_DAILY_LIMIT" "10"
