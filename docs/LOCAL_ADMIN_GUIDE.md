...
Эта инструкция относится только к резервному локальному запуску на Windows.
Единое руководство по работе с админкой и production находится в
[`ADMIN_GUIDE.md`](./ADMIN_GUIDE.md). Скрытая команда бота `/admin` отправляет
именно единое руководство; обычным пользователям команда недоступна.
...
Дополнительные ключи AI, SMTP и Web Push установщик не придумывает. Перенесите их в `backend\.env`; без них основное приложение работает, но соответствующие функции отключены. Пользовательская поддержка хранится в PostgreSQL и не требует отдельного email получателя.
...
3. используйте **Открыть приложение** под новым ответом `/start`.
