...
- `llm`: `llama.cpp` и `Qwen2.5-1.5B-Instruct Q4_K_M` для русских ответов и
  нормализации текста;
...
- Qwen ограничен 1,5 ГБ RAM, 1,5 CPU, контекстом 4096 токенов и одним запросом;
- OCR ограничен 384 МБ RAM, 1 CPU и также обрабатывает одно фото за раз;
- короткий ответ обычно формируется за несколько секунд; при одновременных
  запросах они становятся в очередь;
...
sha256sum /opt/fitness/models/qwen2.5-1.5b-instruct-q4_k_m.gguf
```
...
Источник модели: [официальный репозиторий Qwen](https://huggingface.co/Qwen/Qwen2.5-1.5B-Instruct-GGUF).
